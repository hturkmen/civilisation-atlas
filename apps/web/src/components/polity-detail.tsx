...
export function PolityDetail({boundary, collection, year, headingRef, onBack, onShare, onJump}: {
  ...
  return <div className="detail-panel polity-detail" style={{'--polity-color': polity.color} as React.CSSProperties}>
    ...
    {records.length > 1 && <section className="polity-periods"><p className="eyebrow">BU ALANIN DİĞER DÖNEMLERİ</p><div>{records.map(item => <button key={item.id} aria-pressed={item.id === record.id} onClick={() => onJump(item.sampleYear, polity.id)}>{formatYear(item.sampleYear)}</button>)}</div><p>Yalnız seçilmiş kaynak kayıtları gösterilir. Aradaki boşluklar doldurulmaz.</p></section>}
